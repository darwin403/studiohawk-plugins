...
description: StudioHawk press-release fact-check and audit. Use when a user says things like "audit the press release for a brand", "fact-check the press release", "review the press release before approval", or "check the PR in dpr-approvals". Works from the review thread in the Slack dpr-approvals channel and reads the linked Google Doc, then classifies the piece as data-led, expert-led, or UGC. For data-led pieces it treats the brand's data file and methodology document in Google Drive as the only sources of truth, flagging methodology mismatches, overstated or unsupported claims, wrong statistics, and out-of-scope claims (and can explain any ranking). For every piece it also runs a completeness and compliance check covering the pitch, imagery, client quote, expert credits, brand tie-in, client intro, header logo, Archivo size-11 formatting, and legal soundness. Findings are reported in chat only; it never edits the doc or posts to Slack automatically.
---
